...
int main(int argc, char *argv[])
{
    ...
    int Fmax,iend,Ncurrent,N2add,it2start,iadd,i2add,nESC;
    double *ncount,nepi1,npre1,ndp1,nesc1;
    ...
    double *fesc,*fesc2,*numesc,*numesc2,*besc,*besc2;
    double *tvec;
    ...
    double n0,rnn0,t2start,sdt2start;
    double xcm,ycm,zcm,di2cm,md2cm,rESC;
    ...
    fscanf(input,"%i %s",&N2add,dum);
    fscanf(input,"%i %s",&nESC,dum);
    fscanf(input,"%lg %s",&rESC,dum);
    fscanf(input,"%lg %s",&t2start,dum);
    fscanf(input,"%lg %s",&sdt2start,dum);
    fclose(input);
...
/**********  SIMULATION CONSTANTS  **********/
    ...
    numesc = (double *)calloc(nmetot,sizeof(double));
...
/**********  SIMULATION  **********/
    ...
    for (irun=0;irun<nruns;irun++)
    {
        ...
        dran_gv(rn,1);
        it2start=(int)(t2start*(1.0+sdt2start*rn[0])/dt+0.5);

        it = 0;
        while (it<it2start && Ncells<Nmax)
        {
            for (i=1;i<Ncells;i++)
                for (j=0;j<i;j++)
                {
            ...
            if (it%nme == 0)
            {
              ...
              fesc[it/nme] += (double)nesc1/(double)Ncells;
              ...
              fesc2[it/nme] += (double)(nesc1*nesc1)/(double)(Ncells*Ncells);
              ...
              besc2[Ncells] += (double)(nesc1*nesc1)/(double)(Ncells*Ncells);
              bcount[Ncells] += 1;
              if (irun==0)
              {
            ...
            if (Ncells == N2add)
            {
              for (iadd=0;iadd<nESC;iadd++)
              {
                ...
                for (i=0;i<Ncells;i++)
                {
                  di2cm = sqrt(pow(x[i]-xcm,2.0)+pow(y[i]-ycm,2.0)+pow(z[i]-zcm,2.0));
                  if (di2cm>md2cm)
                  {
                    md2cm = di2cm;
                    i2add = i;
                ...
                if (x[i2add]>xcm)
                  x[Ncells] = x[i2add]+r[i2add];
                if (x[i2add]<=xcm)
                  x[Ncells] = x[i2add]-r[i2add];
                if (y[i2add]>ycm)
                  y[Ncells] = y[i2add]+r[i2add];
                if (y[i2add]<=ycm)
                  y[Ncells] = y[i2add]-r[i2add];
                if (z[i2add]>zcm)
                  z[Ncells] = z[i2add]+r[i2add];
                if (z[i2add]<=zcm)
                  z[Ncells] = z[i2add]-r[i2add];
                vx[Ncells] = vx[i2add];
                vy[Ncells] = vy[i2add];
    ...
    for (i=0;i<nmetot;i++)
        ...
        {
          fprintf(outdat,"%f %f %f %f %f %f %f %f %f %f %f\n",tvec[i],
            fdp[i]/ncount[i],fepi[i]/ncount[i],fpre[i]/ncount[i],
            fesc[i]/ncount[i],
            fdp2[i]/ncount[i],fepi2[i]/ncount[i],fpre2[i]/ncount[i],
            fesc2[i]/ncount[i],
            ntot[i]/ncount[i],ntot2[i]/ncount[i]);
          }

    for (i=1;i<nmetot;i++)
    {
        fprintf(outdat3,"%f %f %f %f %f %f %f %f %f\n",tvec[i],
          numdp[i]/ncount[i],numepi[i]/ncount[i],
          numpre[i]/ncount[i],numesc[i]/ncount[i],numdp2[i]/ncount[i],
          numepi2[i]/ncount[i],numpre2[i]/ncount[i],numesc2[i]/ncount[i]);
    }

    for (i=1;i<Nmax;i++)
    {
        fprintf(outdat4,"%i %f %f %f %f %f %f %f %f\n",i,
          bdp[i]/bcount[i],bepi[i]/bcount[i],
          bpre[i]/bcount[i],besc[i]/bcount[i],bdp2[i]/bcount[i],
          bepi2[i]/bcount[i],bpre2[i]/bcount[i],besc2[i]/bcount[i]);
    }
